...
**Updated 2026-09-22 (pass 3: full-season football accuracy, all-21-sport
source registry, generated fact sheets).** The session's numbers live in
[`docs/FACTS.md`](FACTS.md) (generated, guarded by `tests/test_facts.py`)
and [`docs/SOURCE-REGISTRY.md`](SOURCE-REGISTRY.md) (generated, guarded by
`tests/test_sources.py`).

**What pass 3 added**

- **A real out-of-sample-sized football sample.** The whole 2024/25
  Bundesliga season (306 finished results, ODbL) is committed, so four
  no-market desks are graded on it — 11× the frozen 27-match PnL pilot:
  `elo-favourite-3way-v1` (167 selective calls, **52.1%**, Brier 0.611),
  its always-home baseline (306, 38.6%), `football-totals-poisson-v1`
  (306, **61.1%**, Brier 0.471) and the always-over-2.5 baseline (306,
  59.8%, Brier 0.500). The Elo desk beats always-home, the totals desk
  beats always-over by 1.3 points — marginal, single-source (`probable`),
  no odds, no PnL claim, and both are stated with the sample size that
  produced them.
- **All 21 OLBG sport families now carry an evidence-gated source row**
  (46 pre-registered designs, 27 graded): candidate official source,
  fetched robots.txt text + date, licence state, computed automation gate,
  and the designs waiting behind it. 3 permitted, 17 awaiting licence
  review, 1 blocked by robots.txt (BoxRec). The gate is computed in
  `northstar/sources.py` from those facts; tests fail if a licence is
  called open outside the tested policy registry, if a robots finding
  lacks its quote/date, or if a graded design names a strategy that does
  not exist.
- **Anti-drift documentation.** `docs/FACTS.md` and
  `docs/SOURCE-REGISTRY.md` are written by `python -m northstar.cli facts`
  and checked in CI (`facts --check`); `tests/test_facts.py` and
  `tests/test_sources.py` fail when either is stale. The site's *Sport
  coverage* view renders the same payload with every evidence link
  clickable for manual review, and the Bets view separates priced bets
  (the PnL record) from unpriced calls (`PnL unavailable, never zero`).

**Honest results of the session's new desks** (all single-source
`probable`, all no-odds):
`hockey-elo-mov-v1` 65.5% (255 calls, Brier 0.454) — **below** plain
`hockey-elo-v1` (66.3%, 243), so the margin-of-victory refinement found
...
source gates, test count) are generated into
[`docs/FACTS.md`](FACTS.md) on every pipeline run, and
[`docs/SOURCE-REGISTRY.md`](SOURCE-REGISTRY.md) carries the 21-sport
evidence register. Both are checked by the test suite and by
`python -m northstar.cli facts --check` in CI, so treat them as the
numbers of record and this file as the narrative.

**Updated 2026-09-22 (pass 2: all-21-sport source registry, second-generation
desks, generated fact sheet).** This pass is documented in four places, all
of them generated or tested:

- **[`docs/FACTS.md`](FACTS.md)** — every headline number, regenerated by
  `python -m northstar.cli run-pipeline` / `facts` and guarded by
  `tests/test_facts.py` (a stale sheet fails the suite);
- **[`docs/SOURCE-REGISTRY.md`](SOURCE-REGISTRY.md)** — the source register
  for **all 21 OLBG sport families**, regenerated from
  `data/sources/olbg_sports.json` and revalidated by `tests/test_sources.py`;
- **four new desks** with a **new prediction-only market**: hockey
  `hockey-elo-mov-v1` (margin-of-victory Elo), `hockey-totals-poisson-v1` and
  its baseline `hockey-totals-over-v1` (total goals over/under 5.5, graded
  through a new binary evaluation path) and darts `darts-mov-elo-v1` — 23
  desks in total (13 football PnL + 7 hockey + 3 darts), each carrying a
  naive baseline, none inside the PnL family;
- **a new site view** (*Sport coverage*) rendering every sport's gate,
  robots.txt evidence, licence state and registered designs with clickable
  links for manual review, plus a Bets view that separates priced bets from
  unpriced prediction-only calls.

Current honest results of the new desks (whole DEL 2024/25 season; darts
2025–26 pool; all single-source `probable`, all **no odds path, so no PnL**):
`hockey-elo-mov-v1` grades 65.5% (255 calls, Brier 0.454) and therefore does
**not** beat the plain `hockey-elo-v1` desk (66.3%, 243 calls) — the
margin-of-victory refinement found nothing on this pool; its score stays
above the always-home baseline (59.8%). `hockey-totals-poisson-v1` grades
52.3% (Brier 0.515), **below** its always-over baseline (53.6%, Brier 0.5 by
construction) — the Maher/Dixon-Coles totals model did not transfer to DEL
totals on this season. `darts-mov-elo-v1` grades 78.4% (111 calls, Brier
0.358): more selections than the plain darts desk, lower accuracy. All three
are reported as-is; nothing here is evidence of skill.
...
- **All 21 OLBG sport families carry an evidence-gated source row**
  (`data/sources/olbg_sports.json` → `northstar/sources.py` →
  `docs/SOURCE-REGISTRY.md` + the site's *Sport coverage* view): official
  candidate, fetched robots.txt text and date, licence state, computed
  automation gate and the pre-registered designs behind it. 3 sports are
  permitted (OpenLigaDB ODbL-1.0), 17 await a licence decision, 1 is
  blocked by the source's own robots.txt (BoxRec boxing). A test fails if
  a licence is called open outside the tested policy registry, if a design
  names a strategy that does not exist, or if a robots verdict is missing
  its quote/date.
...
- **Real source irregularities surfaced, not smoothed**: 152 open anomalies
  in the review queue (current value: `docs/FACTS.md`) — 83 DEL matches with impossible result layering
...
- **Seventeen football desks** - thirteen PnL strategies on three markets
  (27-match pilot) plus **four no-market accuracy desks graded on the whole
  committed 2024/25 season (306 matches)** - alongside **seven hockey** and
  **three darts** prediction-only desks. Current per-desk numbers:
  `docs/FACTS.md`.
...
0. **Close the cricket licence question (highest-value new sport).**
   Cricsheet publishes 22,983 matches (JSON/YAML/CSV) and its robots.txt
   allows crawling with only `/data/` excluded from indexing; the licence
   statement on the downloads page has not been transcribed or reviewed.
   If it permits research use, ingest one format (T20 internationals) and
   run the venue/innings-state design in `docs/SOURCE-REGISTRY.md` as
   sport #4 — accuracy-only, because there is still no odds path.
   Every other sport's blocker is written down in the same register with
